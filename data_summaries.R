...
da <- fread("da_type.csv")


# time series
# da[, case_type := factor(case_type, levels = c("initial", "followup", "new"))]
# e_count <- da[, .(count = as.vector(table(case_type)), case_type = levels(da$case_type)), date_unif]
# e_count[, case_type := factor(case_type, levels = c("initial", "followup", "new"))]

da[, date_unif := as.Date(datetime_unif)]
da[, e_type := factor(e_type, levels = c(0, 1))]
e_count <- da[, .(count = as.vector(table(e_type)), e_type = levels(da$e_type)), date_unif]

cols <- c("#721F81FF", "#C2185B", "#E65100")
p <- ggplot(e_count,
...
ggsave("figures/time.png", plot = p, width = 6.5, height = 3.5)

# # weekly time series
# e_count[, week_unif := floor_date(date_unif, "weeks")]
# w_count <- e_count[, .(count = sum(count)), by = .(case_type, week_unif)]
# 
# p <- ggplot(w_count,
#             aes(week_unif, count, color = case_type)) + geom_line() +
#   labs(x = "", y = "Weekly number of events") +
#   scale_color_manual(values = c("initial" = "#721F81FF",
#                                 "followup" = "#C2185B",
#                                 "new" = "#E65100"),
#                      name = "Type of event",
#                      labels = c("Initial call", "Follow-up visit", "New incident")) +
#   theme(legend.position = "bottom",
#         panel.grid.minor = element_blank(),
#         panel.grid.major = element_blank(),
#         legend.margin=margin(t = -0.5, unit='cm'),
#         plot.margin=margin(t = 5.5, r = 5.5, b = 0, l = 5.5, unit="pt")) +
#   guides(colour = guide_legend(title.position = "top"))
# ggsave("figures/time.png", plot = p, width = 5, height = 3.5)

# map with locations and event times
shp <- st_read("cov.shp", crs = 27700, quiet = T)

if (file.exists("roads.shp")){
  roads <- st_read("roads.shp", crs = 27700, quiet = T)
} else {
  # get streets
...
p <- ggplot() +
  ...
  geom_point(data = da, aes(coorx * 1000, coory * 1000, color = days), alpha = 0.4) +
...
ggsave("figures/map.png", plot = p, width = 3.5, height = 4) 
